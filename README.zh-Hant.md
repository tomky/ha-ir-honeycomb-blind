...
Home Assistant 自訂整合元件，使用任何 Home Assistant 紅外線遙控器實體控制**上下合蜂巢簾**（Top-Down Bottom-Up Honeycomb Blind）。已在 Broadlink 上測試通過。
...
- **共享遙控器**：多組窗簾可共用同一台紅外線遙控器，IR 指令自動排隊
...
- **紅外線遙控器整合**：任何提供 `remote` 實體的整合（例如 Broadlink、Tuya、小米、SwitchBot、SmartIR）
- **IR 遙控碼**：已學習好窗簾遙控器的 5 個按鈕 IR 碼（格式依遙控器整合而定）
...
| 窗簾名稱 | 為此窗簾取一個獨特的名稱 |
| 遙控器 | 選擇要使用的遙控器實體 |
| IR 碼 (T-UP/T-DN/B-UP/B-DN/STOP) | 貼上 IR 碼（Broadlink 使用 `b64:` 格式） |
| 全開時間 | 窗簾從全關到全開所需秒數 |
| 全關時間 | 窗簾從全開到全關所需秒數 |
...
### 多個窗簾互相干擾
- 確認每組窗簾使用不同的 IR 碼
- 如果共用同一台紅外線遙控器，IR 指令會自動排隊，請耐心等待
